...
func TestDiagnosticsOnDidChange(t *testing.T) {
	client := NewTestClient(t)
	defer client.Close()
...
func TestDiagnosticsOnDidChangeEmpty(t *testing.T) {
	client := NewTestClient(t)
	defer client.Close()
	...
	// 2. Simulate opening a bad GDScript file
	client.DidOpen("file:///test.gd", "func missing_return() -> void:\n  pass")
	...
	if len(diags.Diagnostics) != 0 {
		t.Errorf("Expected 0 diagnostics, got %d", len(diags.Diagnostics))
	}
}
